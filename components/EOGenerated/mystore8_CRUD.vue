...
const TOKENIZED = {
  ALL: 'mystore8/all',
  UPDATE: 'mystore8/update',
  SCHEMA_DISPLAY_DEFINITION: 'mystore8/schemaDsiplayDefinition',
  STORE_NAME: 'mystore8',
}
...
export default {
  ...
  methods: {
    updateItem(fieldDef, id, e) {
      this.$store.dispatch(TOKENIZED.UPDATE, {
        fieldDef,
        id: id,
        newValue: e,
      })
  ...
  computed: {
    ...mapGetters({
      schemaDsiplayDefinition: TOKENIZED.SCHEMA_DISPLAY_DEFINITION,
      all: TOKENIZED.ALL,
    }),
    comp() {
      return this.$store.getters[TOKENIZED.SCHEMA_DISPLAY_DEFINITION]
    },
    headers() {
  ...
  data() {
    return {
      selectedItem: null,
      selectedId: null,
      storeName: TOKENIZED.STORE_NAME,
    }
  },
}
</script>
